...
summarize_matrix <- function(x, na.rm=FALSE) {
  #Apply a function to each row of the matrix to calculate the required statistics
  row_summaries <- apply(x, 1, function(row) {
    #Calculate individual statistics
    mean_val <- mean(row, na.rm = TRUE)
    stdev_val <- sd(row, na.rm = TRUE)
    ...
    c(mean = mean_val, stdev = stdev_val, median = median_val, min = min_val,
      max = max_val, num_lt_0 = num_lt_0, num_btw_1_and_5 = num_btw_1_and_5, num_na = num_na)
  })
  
  #Convert the result to a data frame with each row as a summary
  row_summaries_df <- as.data.frame(t(row_summaries))
   
   
  return(row_summaries_df)
}
